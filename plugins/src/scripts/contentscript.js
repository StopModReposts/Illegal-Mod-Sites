chrome.runtime.sendMessage(0, function(smr_hosts) {
    var smr_illegal = false;
    var smr_host = location.host.replace(/\.blogspot\.[a-z]+(\.[a-z]+)?$/, ".blogspot.tld");
    ...
    for (var smr_iterator = 0; smr_iterator < smr_hosts.length; smr_iterator++) {
        if ((smr_host.length > smr_hosts[smr_iterator].domain.length && smr_host.substr(smr_host.length - smr_hosts[smr_iterator].domain.length - 1) == "." + smr_hosts[smr_iterator].domain) || (smr_host.length == smr_hosts[smr_iterator].domain.length && smr_host == smr_hosts[smr_iterator].domain)) {
            var smr_regexp = new RegExp(smr_hosts[smr_iterator].pattern);
            if (smr_regexp.test(location.href)) {
                smr_illegal = true;
                smr_ratings = [smr_hosts[smr_iterator].advertising - 1, smr_hosts[smr_iterator].redistribution - 1, smr_hosts[smr_iterator].miscellaneous - 1];
                smr_notes = smr_hosts[smr_iterator].notes;
            }
        }
    }
    ...
    if (smr_overlay_element == null && smr_illegal) { // Ensure that the overlay doesn't already exist
        ...
        if (smr_notes.length > 0) {
        
                    var smr_rating_notes = smr_base_p.cloneNode(false);
                    smr_rating_notes.style.marginBottom = "30px";
                    smr_rating_notes.textContent = chrome.i18n.getMessage("site_notes", smr_notes);
